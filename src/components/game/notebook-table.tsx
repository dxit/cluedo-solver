...
function NotebookStatusButton({
	...
	return (
		<Button
			...
			size="sm"
			className={cn(
				"h-9 min-w-12 rounded-full border px-3 font-semibold shadow-none",
				statusTone[status],
			)}
			onClick={() => onStatusChange(card, columnKey, statusCycle[status])}
...
export default function NotebookTable({
	...
	const columns = useMemo<ColumnDef<NotebookRow>[]>(
					...
					<div className="flex justify-center">
						<NotebookStatusButton
							...
							ariaLabel={t("notebook.setStatus", {
								...
								column: player.name,
								status: t(
									`notebook.status.${statusCycle[notebook[row.original.card]?.[player.id] ?? "unknown"]}`,
								),
							})}
					...
					<div className="flex justify-center">
						<NotebookStatusButton
							...
							ariaLabel={t("notebook.setStatus", {
								...
								status: t(
									`notebook.status.${statusCycle[notebook[row.original.card]?.[envelopeColumnId] ?? "unknown"]}`,
								),
							})}
							onStatusChange={onStatusChange}
						/>
		...
		[players, notebook, onStatusChange, t],
	);
